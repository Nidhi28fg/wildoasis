...
const Stat: FC<StatProps> = ({
  ...
  return (
    <a href={link}>
      <div className="bg-white dark:bg-black border border-gray-100 dark:border-gray-800 rounded-lg p-4 grid grid-cols-[6rem_1fr] grid-rows-[auto_auto] gap-x-4 gap-y-1">
        <div
          className={`stat-icon  ${color} row-span-full aspect-[1] rounded-full flex items-center justify-center `}
        >
          {<Icon className={`w-7 h-7`} />}
        </div>
        <h5 className="self-end text-[1rem] uppercase font-semibold text-gray-500  dark:text-gray-400 tracking-[0.1rem]">
          {title}
        </h5>
